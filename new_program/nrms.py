...
from layers import AttLayer2, SelfAttention, SelfAttentions
...
class NRMSModel(BaseModel):
    ...
    def _build_newsencoder(self, embedding_layer):
        ...
        y = SelfAttentions(hparams.output_dim, seed=self.seed)([y, y, y])
        y = layers.Dropout(hparams.dropout)(y) # Tensor("dropout_1/cond/Merge:0", shape=(?, 30, 400), dtype=float32) for selfattentions
        pred_title = AttLayer2(hparams.attention_hidden_dim, seed=self.seed)(y)

        model = keras.Model(sequences_input_title, pred_title, name="news_encoder") # predtitle: (?, 400)
        return model
    ...
    def _build_nrms(self):
        ...
        titleencoder = self._build_newsencoder(embedding_layer) # [<tf.Tensor 'att_layer2/Sum_1:0' shape=(?, 400) dtype=float32>]
        self.userencoder = self._build_userencoder(titleencoder)
        print("encoder")
        self.newsencoder = titleencoder

        user_present = self.userencoder(his_input_title)# should be Tensor("user_encoder/att_layer2_1/Sum_1:0", shape=(?, 400), dtype=float32)
        news_present = layers.TimeDistributed(self.newsencoder)(pred_input_title)  # should be Tensor("time_distributed_1/Reshape_1:0", shape=(?, 5, 400), dtype=float32)
        # news_present = self.userencoder(pred_input_title)
