...
class SelfAttentions(layers.Layer):
    """Multi-head self attention implement.

    Args:
        multiheads (int): The number of heads.
        head_dim(obj): Dimension of each head.
        mask_right(boolean): whether to mask right words.
    ...
    def __init__(self, output_dim, seed=0, **kwargs):
        self.output_dim = output_dim
        self.seed = seed
        self.i = 1
        super(SelfAttentions, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(SelfAttentions, self).build(input_shape)
    ...
    def call(self, QKVs):
        ...
        WQ = K.dot(Q_seq, self.WQ)
        WK = K.dot(K_seq, self.WK)
        WV = K.dot(V_seq, self.WV)

        print("WQ.shape", WQ.shape, self.i)
        self.i+=1

        print("K.permute_dimensions(WK, [0, 2, 1]).shape", K.permute_dimensions(WK, [0, 2, 1]).shape)

        QK = K.batch_dot(WQ, K.permute_dimensions(WK, [0, 2, 1]))
        QK = QK / K.sqrt(K.cast(self.output_dim, dtype="float32"))
        QK = K.softmax(QK)
        V = K.batch_dot(QK, WV)
