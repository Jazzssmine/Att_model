...
class BaseModel(object):
    ...
    def fit(
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            if test_news_file is not None:
                test_res = self.run_test(test_news_file, test_behaviors_file)
                test_info = ", ".join(
                    [
                        str(item[0]) + ":" + str(item[1])
    ...
    def run_test_news(self, news_filename):
        if not hasattr(self, "newsencoder"):
            raise ValueError("model must have attribute newsencoder")
        ...
        news_vecs = []
        for batch_data_input in tqdm(
            self.test_iterator.load_test_news_from_file(news_filename)
        ):
            news_index, news_vec = self.news(batch_data_input)
            news_indexes.extend(np.reshape(news_index, -1))
            news_vecs.extend(news_vec)
    ...
    def run_test_user(self, news_filename, behaviors_file):
        if not hasattr(self, "userencoder"):
            raise ValueError("model must have attribute userencoder")
        ...
        user_vecs = []
        for batch_data_input in tqdm(
            self.test_iterator.load_test_user_from_file(news_filename, behaviors_file)
        ):
            user_index, user_vec = self.user(batch_data_input)
            user_indexes.extend(np.reshape(user_index, -1))
            user_vecs.extend(user_vec)
    ...
    def run_test(self, news_filename, behaviors_file):
        news_vecs = self.run_test_news(news_filename)
        user_vecs = self.run_test_user(news_filename, behaviors_file)
        ...
        for (
            ...
            news_index,
            user_index,
        ) in tqdm(self.test_iterator.load_test_impression_from_file(behaviors_file)):
            pred = np.dot(
                np.stack([news_vecs[i] for i in news_index], axis=0),
                user_vecs[impr_index],
            )
        ...
        return group_impr_indexes, group_preds
